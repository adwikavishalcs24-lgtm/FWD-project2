...
export const AncientClockmakerAlignment = ({
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setGears(prev => {
        const newGears = prev.map(gear => ({
          ...gear,
          angle: (gear.angle + gear.speed) % 360
        }));

        // Check alignment
        const isAligned = newGears.every(g => Math.abs(g.speed - g.targetSpeed) < 0.1);

        // This setter is safe here, but we should probably lift logic out if possible. 
        // However, we need access to 'newGears'.
        // To avoid loop complexity with setAligned inside setGears update, 
        // we'll rely on the next render for 'aligned' update, or just use a ref for aligned status?
        // Actually, we can just update 'aligned' state here? No, calling setAligned inside setGears updater is bad.
        // We will move the alignment check to a separate derived value during render or effect?
        // Optimization: Just calculate it here and use a ref for "isAligned" to read in the scoring phase?
        // Simpler: Just do it in the next effect or accept the flicker. Ref is better.
        return newGears;
      });
    }, 16);
  ...
  // Separate effect for checking alignment and scoring
  useEffect(() => {
    const isAligned = gears.every(g => Math.abs(g.speed - g.targetSpeed) < 0.1);
    setAligned(isAligned);

    if (isAligned && gameRef.current?.isGameStarted) {
      // Add points periodically
      const now = Date.now();
      // We can use a modulo or random chance, or just accumulate
      // Since this runs whenever 'gears' changes (every 16ms approx from internal state update?),
      // Actually the above useEffect updates gears 60fps.
      // This useEffect depends on 'gears'.

      if (Math.random() < 0.05) { // Approx once per second? (0.05 * 60 = 3 times/sec). 
        // Let's hold for 1 second of alignment = good points
        gameRef.current.addPoints(5, 100, 100, 'good');
      }
    }
  }, [gears]);
  ...
  const renderGear = (gear) => (
    <div key={gear.id} className="flex flex-col items-center gap-4">
      ...
      <div className="flex gap-2">
        ...
        <div className={`px-2 py-1 rounded text-xs font-bold flex items-center ${Math.abs(gear.speed - gear.targetSpeed) < 0.1 ? 'bg-green-900 text-green-400' : 'bg-gray-800 text-gray-400'
          }`}>

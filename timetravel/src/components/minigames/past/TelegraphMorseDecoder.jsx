...
export const TelegraphMorseDecoder = ({
  ...
  const generateNewWord = () => {
    // Basic word selection needed avoiding immediate repeats?
  ...
  const playMorseSound = () => {
    ...
    currentWord.split('').forEach((char, idx) => {
      ...
      delay += 1000; // 1 second per character for readability
    });
    ...
    >
      <div className="w-full h-full flex flex-col items-center justify-center p-4">
          ...
          <div className="flex justify-center gap-8">
            {morseDisplay.map((item, idx) => (
              <div key={idx} className={`flex flex-col items-center transition-all duration-300 ${item.active ? 'scale-125 text-amber-800 font-bold' : 'opacity-70'}`}>
                ...
                {/* Always show question mark or hint? 
                            Let's keep question mark as per original design, but maybe flash letter on playing?
                        */}
                <div className="text-sm text-gray-500 opacity-50">?</div>
              </div>
            ))}
          </div>
          {isPlaying && <div className="absolute bottom-2 right-2 text-xs text-amber-700 animate-pulse">TRANSMITTING...</div>}
        </div>
        ...
        <div className="mt-6 flex gap-4">
          ...
          <button onClick={playMorseSound} className="text-xs text-amber-500 border border-amber-500 p-2 rounded hover:bg-amber-900">
            REPLAY SIGNAL
          </button>
        </div>

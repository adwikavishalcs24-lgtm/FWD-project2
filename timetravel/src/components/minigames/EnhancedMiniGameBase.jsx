...
export const MiniGameBase = ({
  ...
  const addPoints = (basePoints, x, y, type = 'score') => {
    ...
    setGameStats(prev => ({
      ...
      perfectActions: type === 'perfect' ? prev.perfectActions + 1 : prev.perfectActions
    }));
  ...
  const endGame = async () => {
    ...
    try {
      ...
      await submitMiniGameScore(timeline, title, gameState.score, {
        ...
        attempts: 1,
        perfectActions: gameStats.perfectActions,
        totalActions: gameStats.totalActions,
  ...
  const calculateEnhancedRewards = () => {
    ...
    return {
      ...
      stability: Math.floor(accuracy * 20) - Math.floor((duration - timeLeft) / 10),
      coinsPerSecond: Math.floor(maxCombo / 2),
      resources: {
        timelineInfluence: Math.floor(perfectActions / 2),
        performanceBonus: Math.floor(comboBonus / 100)
      }
    };
  };
  ...
  useEffect(() => {
    const comboTimer = setInterval(() => {
      if (Date.now() - lastPerfectTime > 3000) {
        setComboMultiplier(1);
      }
    }, 1000);
        ...
        <div className="scoring-card">
          ...
          <div className="scoring-details">
            ...
            <div className="scoring-item">
              ...
              <span>Reset combo multiplier</span>
            </div>
          </div>
        </div>
      </div>
